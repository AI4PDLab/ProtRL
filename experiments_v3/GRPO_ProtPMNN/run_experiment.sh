...
do
    ...
    # Run ProteinMPNN (via Apptainer)
    # Run ProteinMPNN (via Apptainer)
    echo "Running ProteinMPNN..."
    mkdir -p "$results_dir/ProteinMPNN_scores"
    
    for pdb_file in "$results_dir/PDB"/*.pdb; do
        if [ -f "$pdb_file" ]; then
            pdb_name=$(basename "$pdb_file" .pdb)
            # Run ProteinMPNN directly for each file
            apptainer exec --nv ../../../apptainers/proteinmpnn.sif python /app/ProteinMPNN/protein_mpnn_run.py \
                --pdb_path "$pdb_file" \
                --pdb_path_chains "A" \
                --score_only 1 \
                --save_score 1 \
                --out_folder "$results_dir/ProteinMPNN_scores" \
                --batch_size 1 > /dev/null 2>&1
        fi
    done
