...
def main():
    ...
    for i, output_ids in enumerate(tqdm(generated_ids)):
        ...
        sequence_text = tokenizer.decode(output_ids, skip_special_tokens=False).replace(" ","")
        # Extract the actual sequence between prompt and end tokens
        # The prompt is hardcoded as {label}<sep><start>
        # We replace the known prompt and potential end tokens
        sequence_text = sequence_text.replace(f"{args.label}<sep><start>", "").replace("<end>", "").replace("<|endoftext|>", "")
        # Calculate metrics
