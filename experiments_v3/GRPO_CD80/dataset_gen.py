...
def load_model_and_tokenizer():
    ...
    model_name ="/home/woody/b114cb/b114cb23/models/FT_ZymCTRL_altals_2/output_3" # put the path to your local model or a Huggingface's repository (to be called with transformer's API)
...
def append_to_csv(**kwargs):
    ...
    fieldnames = [  
      "name","pMPNN","d_rmsd","pAE_bt","shape_complimentary","uns_hydrogens",
      "hydrophobicity","binder_score","interface_dSASA","iteration_num",
      "plddt","i_pTM","pAE_b","ipsae","helicity","lenght","ptm","has_clash",
      "pAE_t","pae_2","delta_delta_interaction","binder_sasa","score",
      "sequence","contact_probs","i_pae"
    ]
...
def af_metrics(name, path):
    ...
    i_pae = metrics_summary['chain_iptm'][0]
    ...
    return iptm, pae , plddt, ptm, has_clash, ipsae, b_pae, t_pae, pae_2, i_pae
...
def generate_dataset(iteration_num, label, mode, array):
    ...
    seq_lenght = []
    with open(f"seq_gen_{label}_iteration{iteration_num}.fasta", "r") as f:
        rep_seq = f.readlines()
    ...
    try:
        for entry in chunks[array]:
                ...
                path = f"./alphafold_output_iteration{iteration_num}/{name.lower()}"
                i_pTM, pae , plddt, ptm, has_clash, ipsae, pAE_b, pAE_t, pae_2, i_pae = af_metrics(name.lower(), path)
                ...
                output_file = f"logs_output_iteration_{iteration_num}.csv"
                append_to_csv(
                        name                = name,
                        pMPNN               = -pMPNN,
                        d_rmsd              = d_rmsd,
                        ...
                        score               = score,
                        sequence            = formatting_sequence(sequence, label),
                        i_pae               = i_pae,
...
if __name__ == "__main__":
    ...
    seed_everything(42)
    model, modelre, toke = load_model_and_tokenizer()
