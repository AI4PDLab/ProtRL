...
class weighted_DPO(GRPOTrainer):
        ...
        *,
        ...
        if self.beta == 0.0:
            ...
            self.ref_model = None
             ...
             # Fallback if ref_model was not set above
             pass
    ...
    def _compute_loss(self, model, inputs, return_outputs=False):
        ...
        # Calculate implicit rewards (log ratio)
        # u_i = beta * (log_pi - log_ref)
        if torch.allclose(ref_per_token_logps, per_token_logps, rtol=1e-5, atol=1e-8):
             # If models are identical, ratio is 0 (log(1))
             log_ratio = torch.zeros_like(per_token_logps.mean(dim=1))
        else:
             log_ratio = per_token_logps.mean(dim=1) - ref_per_token_logps.mean(dim=1)
        
        u = self.beta * log_ratio

        # Calculate target weights w_i from rewards
        # Assuming rewards are unnormalized scores, we softmax them to get weights summing to 1
        # The formula uses w^(i) which usually sum to 1 in this context (Cross Entropy)
        w = F.softmax(rewards, dim=0)

        # Compute Loss: - sum(w_i * u_i) + log(sum(exp(u_i)))
        # This is exactly CrossEntropyLoss(input=u, target=w) where u is logits and w is probabilities
        # We need to unsqueeze to make them (1, batch_size) as CrossEntropy expects (N, C)
        loss = F.cross_entropy(u.unsqueeze(0), w.unsqueeze(0))
